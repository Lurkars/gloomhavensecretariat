...
@Component({
  ...
  imports: [NgClass, SlicePipe, FormsModule, InViewportDirective, AutoscrollDirective, GhsLabelDirective, TabClickDirective],
...
export class CampaignHistoryComponent implements OnInit {
  ...
  displayedEntries(): CampaignHistoryEntry[] {
    ...
    return this.filteredEntries().slice(0, this.displayCount);
  ...
  moreEntries() {
    ...
    const filteredLength = this.filteredEntries().length;

    if (this.displayCount < filteredLength) {

      this.displayCount = Math.min(filteredLength, this.displayCount + 50);
  ...
  private scenarioRecordFromNearby(entry: CampaignHistoryEntry): string {
    ...
    for (let i = index + 1; i < this.entries.length && i <= index + 30; i++) {
      ...
      if (
        ...
        nearbyKey.startsWith('finishScenario.') &&

        nearbyKey !== 'finishScenario.battleGoal' &&

        nearbyKey !== 'finishScenario.trial'
