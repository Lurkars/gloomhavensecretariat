...
export class EnhancementsComponent implements OnInit {
    ...
    constructor(private ghsManager: GhsManager) {
        this.ghsManager.uiChangeEffect(() => {
            if (this.inputCharacter && this.inputCardId) {
                this.enhancements = this.inputCharacter.progress.enhancements && this.inputCharacter.progress.enhancements.filter((enhancement) => enhancement.cardId == this.inputCardId && this.inputActionIndex && enhancement.actionIndex.indexOf('bottom') == this.inputActionIndex.indexOf('bottom')).length || 0;
                this.enhancedCards = this.inputCharacter.progress.enhancements && this.inputCharacter.progress.enhancements.filter((e) => !e.inherited).map((enhancement) => enhancement.cardId).filter((cardId, index, self) => (!this.inputCardId || this.inputCardId != cardId) && index == self.indexOf(cardId)).length;
            }
        });
    }
    ...
    ngOnInit(): void {
        ...
        if (this.inputAction && this.inputActionIndex && this.inputCardId && this.inputEnhancementIndex != undefined && this.inputCharacter) {
            ...
            this.enhancedCards = this.inputCharacter.progress.enhancements && this.inputCharacter.progress.enhancements.filter((e) => !e.inherited).map((e) => e.cardId).filter((cardId, index, self) => (!this.inputCardId || this.inputCardId != cardId) && index == self.indexOf(cardId)).length;

            if ([ActionType.custom, ActionType.specialTarget].includes(this.inputAction.type) && this.inputAction.enhancementTypes && !this.wipSpecial) {
    ...
    update() {
        ...
        if (this.inputActionIndex && this.inputCardId && this.inputEnhancementIndex != undefined && this.character) {
            this.character.tags = [];
            this.character.progress.enhancements = this.inputCharacter && this.inputCharacter.progress.enhancements && JSON.parse(JSON.stringify(this.inputCharacter.progress.enhancements)) || [];

            this.character.progress.enhancements.push(new Enhancement(this.inputCardId, this.inputActionIndex, this.inputEnhancementIndex, this.enhancementAction));
        }
    ...
    apply(force: boolean = false) {
        ...
        if (this.inputActionIndex && this.inputCardId && this.inputEnhancementIndex != undefined && this.inputCharacter && (this.inputCharacter.progress.gold >= costs && (gameManager.enhancementsManager.fh || this.enhancedCards < gameManager.prosperityLevel()) || force)) {
            gameManager.stateManager.before('enhanceCard', gameManager.characterManager.characterName(this.inputCharacter, true, true), this.inputCardId);

            if (!this.inputCharacter.progress.enhancements) {
                this.inputCharacter.progress.enhancements = [];
            }

            this.inputCharacter.progress.enhancements.push(new Enhancement(this.inputCardId, this.inputActionIndex, this.inputEnhancementIndex, this.enhancementAction));
