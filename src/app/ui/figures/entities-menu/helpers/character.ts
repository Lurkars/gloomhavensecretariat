...
import { CharacterLevelDialogComponent } from "../level-dialog/level-dialog";
import { SummonDialogComponent } from "../summon-dialog/summon-dialog";
...
export class CharacterHelper {
    ...
    setIdentity(index: number) {
        if (this.component.entity instanceof Character && index != this.component.entity.identity) {
            let timeTokens = this.component.entity.name == 'blinkblade' && this.component.entity.tags.includes('time_tokens') && this.component.entity.primaryToken == 0;
            if ((gameManager.game.state == GameState.next || gameManager.game.state == GameState.draw && this.component.entity.identity == 0 && this.component.entity.tokenValues[0] == 0) && timeTokens) {
                return;
            }
            this.component.before("identity", this.component.entity.edition, this.component.entity.name, index, this.component.entity.identities[index]);
    ...
    openLevelDialog() {
        if (this.component.figure instanceof Character && (this.component.entity instanceof Character || this.component.entity instanceof Summon)) {
            const dialogData = {
                ...
                positionStrategy: this.component.data.positionElement && this.component.overlay.position().flexibleConnectedTo(this.component.data.positionElement).withPositions(ghsDefaultDialogPositions())
            };
            if (this.component.entity instanceof Character) {
