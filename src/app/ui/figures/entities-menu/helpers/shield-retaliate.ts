...
export class ShieldRetaliateHelper {
  ...
  update() {
    this.component.shieldAndRetaliate = settingsManager.settings.characterShieldRetaliate && (this.component.figures.find((figure) => figure instanceof Character) != undefined || this.component.figures.find((figure) => figure instanceof ObjectiveContainer && figure.escort) != undefined) || settingsManager.settings.standeeShieldRetaliate && this.component.figures.find((figure) => figure instanceof Monster) != undefined;
  ...
  changeRetaliatePersistent(index: number, value: number, range: number, remove: boolean = false) {
    ...
    this.component.entityRetaliatePersistent[index].value = EntityValueFunction(this.component.entityRetaliatePersistent[index].value) + value;
    this.component.entityRetaliateRangePersistent[index].value = EntityValueFunction(this.component.entityRetaliateRangePersistent[index].value) + range;
  ...
  close() {
    if (this.component.entities.some((entity) => EntityValueFunction(this.component.entityShield.value) > 0 && !entity.shield || entity.shield && EntityValueFunction(entity.shield.value) != EntityValueFunction(this.component.entityShield.value))) {
      this.component.before('changeShield', this.component.entityShield.value);
      this.component.entities.forEach((entity) => {
        if (entity instanceof Character && settingsManager.settings.characterShieldRetaliate || !(entity instanceof Character) && settingsManager.settings.standeeShieldRetaliate) {
    ...
    if (this.component.entities.some((entity) => EntityValueFunction(this.component.entityShieldPersistent.value) > 0 && !entity.shieldPersistent || entity.shieldPersistent && EntityValueFunction(entity.shieldPersistent.value) != EntityValueFunction(this.component.entityShieldPersistent.value))) {
      ...
      this.component.entities.forEach((entity) => {
        if (entity instanceof Character && settingsManager.settings.characterShieldRetaliate || !(entity instanceof Character) && settingsManager.settings.standeeShieldRetaliate) {
          if (!this.component.entity && entity.shieldPersistent) {
            entity.shieldPersistent.value = EntityValueFunction(entity.shieldPersistent.value) + EntityValueFunction(this.component.entityShieldPersistent.value);
          } else {
    ...
    if (retaliate.length > 0 && retaliate.some((retaliateAction) => this.component.entities.some((entity) => EntityValueFunction(retaliateAction.value) > 0 && !entity.retaliate.some((action) => JSON.stringify(retaliateAction) == JSON.stringify(action)) || entity.retaliate.length && entity.retaliate.some((action) => JSON.stringify(action.subActions) == JSON.stringify(retaliateAction.subActions) && EntityValueFunction(action.value) != EntityValueFunction(retaliateAction.value))))) {
      this.component.before('changeRetaliate', retaliate.map((action) => '%game.action.retaliate% ' + EntityValueFunction(action.value) + (action.subActions && action.subActions[0] && action.subActions[0].type == ActionType.range && EntityValueFunction(action.subActions[0].value) > 1 ? ' %game.action.range% ' + EntityValueFunction(action.subActions[0].value) : '')).join(', '));
      this.component.entities.forEach((entity) => {
        if (entity instanceof Character && settingsManager.settings.characterShieldRetaliate || !(entity instanceof Character) && settingsManager.settings.standeeShieldRetaliate) {
    ...
    if (retaliatePersistent.length > 0 && retaliatePersistent.some((retaliatePersitentAction) => this.component.entities.some((entity) => EntityValueFunction(retaliatePersitentAction.value) > 0 && !entity.retaliatePersistent.some((action) => JSON.stringify(retaliatePersitentAction) == JSON.stringify(action)) || entity.retaliatePersistent.length && entity.retaliatePersistent.some((action) => JSON.stringify(action.subActions) == JSON.stringify(retaliatePersitentAction.subActions) && EntityValueFunction(action.value) != EntityValueFunction(retaliatePersitentAction.value))))) {
      this.component.before('changeRetaliatePersistent', retaliatePersistent.map((action) => '%game.action.retaliate% ' + EntityValueFunction(action.value) + (action.subActions && action.subActions[0] && action.subActions[0].type == ActionType.range && EntityValueFunction(action.subActions[0].value) > 1 ? ' %game.action.range% ' + EntityValueFunction(action.subActions[0].value) : '')).join(', '));
      this.component.entities.forEach((entity) => {
        if (entity instanceof Character && settingsManager.settings.characterShieldRetaliate || !(entity instanceof Character) && settingsManager.settings.standeeShieldRetaliate) {
