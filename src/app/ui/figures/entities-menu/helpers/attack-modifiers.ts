...
export class AttackModifierHelper {
  ...
  updateEmpowerEnfeeble(updateChars: boolean = true) {
    if (updateChars) {
      this.component.empowerChars = gameManager.game.figures.filter((figure) => figure instanceof Character && gameManager.entityManager.isAlive(figure) && figure.additionalModifier && figure.additionalModifier.find((perk) => perk.attackModifier && perk.attackModifier.type == AttackModifierType.empower)).map((figure) => figure as Character);
    ...
    if (this.component.figure) {
      ...
      this.component.empowerMax = this.component.empowerChar ? this.component.empowerChar.additionalModifier.filter((perk) => perk.attackModifier && perk.attackModifier.type == AttackModifierType.empower).map((perk) => perk.count).reduce((a, b) => a + b) - gameManager.attackModifierManager.countUpcomingAdditional(this.component.empowerChar, AttackModifierType.empower) : -1;
      this.component.enfeebleMin = this.countUpcomingAttackModifier(amDeck, AttackModifierType.enfeeble);
      this.component.enfeebleMax = this.component.enfeebleChar ? this.component.enfeebleChar.additionalModifier.filter((perk) => perk.attackModifier && perk.attackModifier.type == AttackModifierType.enfeeble).map((perk) => perk.count).reduce((a, b) => a + b) - gameManager.attackModifierManager.countUpcomingAdditional(this.component.enfeebleChar, AttackModifierType.enfeeble) : -1;
    } else {
  ...
  updateAmDecks() {
    ...
    this.component.amDecks.push(...gameManager.game.figures.filter((figure) => figure instanceof Character && !figure.absent).map((figure) => figure.name));

    if (this.component.figure instanceof ObjectiveContainer && this.component.figure.amDeck && !this.component.amDecks.includes(this.component.figure.amDeck)) {
  ...
  countUpcomingAttackModifier(amDeck: AttackModifierDeck, type: AttackModifierType, idPrefix: string | undefined = undefined): number {
    return amDeck.cards.filter((attackModifier, index) => attackModifier.type == type && index > amDeck.current && (!idPrefix || attackModifier.id && attackModifier.id.startsWith(idPrefix))).length;
  }
  ...
  changeAttackModifier(entity: Entity, type: AttackModifierType, value: number) {
    ...
    const isMonster = figure instanceof Monster && (!figure.isAlly && !figure.isAllied || !settingsManager.settings.alwaysAllyAttackModifierDeck && !gameManager.fhRules()) || figure instanceof ObjectiveContainer && figure.amDeck == 'M'
    if (value > 0) {
      for (let i = 0; i < value; i++) {
        if (type == AttackModifierType.bless && gameManager.attackModifierManager.countUpcomingBlesses() < 10 || type == AttackModifierType.curse && gameManager.attackModifierManager.countUpcomingCurses(isMonster) < 10) {
  ...
  close() {
    if (this.component.bless != 0) {
      this.component.before(this.component.bless < 0 ? "removeCondition" + (this.component.bless < -1 ? 's' : '') : "addCondition" + (this.component.bless > 1 ? 's' : ''), AttackModifierType.bless, (this.component.bless > 0 ? this.component.bless : this.component.bless * -1));
    ...
    if (this.component.curse != 0) {
      this.component.before(this.component.curse < 0 ? "removeCondition" + (this.component.curse < -1 ? 's' : '') : "addCondition" + (this.component.curse > 1 ? 's' : ''), AttackModifierType.curse, (this.component.curse > 0 ? this.component.curse : this.component.curse * -1));
      this.component.entities.forEach((entity) => {
    ...
    if (this.component.empower != 0) {
      if (this.component.empowerChar || this.component.empower < 0) {
        this.component.before(this.component.empower < 0 ? "removeCondition" + (this.component.empower < -1 ? 's' : '') : "addCondition" + (this.component.empower > 1 ? 's' : ''), AttackModifierType.empower, (this.component.empower > 0 ? this.component.empower : this.component.empower * -1));
        this.component.entities.forEach((entity) => {
          const figure = this.component.figureForEntity(entity);
          const amDeck = gameManager.attackModifierManager.byFigure(figure);
          if (this.component.empowerChar && this.component.empower > 0) {
            ...
            for (let i = 0; i < Math.min(this.component.empower, additional.length); i++) {
              const count = this.component.empowerChar.additionalModifier.filter((perk) => perk.attackModifier && perk.attackModifier.type == AttackModifierType.empower).map((perk) => perk.count).reduce((a, b) => a + b) - gameManager.attackModifierManager.countUpcomingAdditional(this.component.empowerChar, AttackModifierType.empower);
              if (count > 0) {
    ...
    if (this.component.enfeeble != 0) {
      if (this.component.enfeebleChar || this.component.enfeeble < 0) {
        this.component.before(this.component.enfeeble < 0 ? "removeCondition" + (this.component.enfeeble < -1 ? 's' : '') : "addCondition" + (this.component.enfeeble > 1 ? 's' : ''), AttackModifierType.enfeeble, (this.component.enfeeble > 0 ? this.component.enfeeble : this.component.enfeeble * -1));
        this.component.entities.forEach((entity) => {
          const figure = this.component.figureForEntity(entity);
          const amDeck = gameManager.attackModifierManager.byFigure(figure);
          if (this.component.enfeebleChar && this.component.enfeeble > 0) {
            ...
            for (let i = 0; i < Math.min(this.component.enfeeble, additional.length); i++) {
              const count = this.component.enfeebleChar.additionalModifier.filter((perk) => perk.attackModifier && perk.attackModifier.type == AttackModifierType.enfeeble).map((perk) => perk.count).reduce((a, b) => a + b) - gameManager.attackModifierManager.countUpcomingAdditional(this.component.enfeebleChar, AttackModifierType.enfeeble);
              if (count > 0) {
                gameManager.attackModifierManager.addModifier(amDeck, additional[i]);
              }
            }
