...
import { DistributionRow, EventManualDistributionHelper } from 'src/app/ui/figures/entities-menu/helpers/event-manual-distribution';
...
export class EventManualDistributionComponent {
  ...
  label(key: string, args?: (string | number)[]): string {
    return settingsManager.getLabel(
      key,
      args?.map((a) => '' + a)
    );
  }
