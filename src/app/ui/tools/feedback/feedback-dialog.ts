...
export class FeedbackDialogComponent {
  ...
  scenarioMail(scenarioName: string, index: string, notes: string): string {
    ...
    const scenario = new ScenarioData();
    ...
    scenario.allies = gameManager.game.figures
      .filter((figure) => figure instanceof Monster && figure.isAlly)
      .map((figure) => (figure as Monster).name);
    scenario.allied = gameManager.game.figures
      .filter((figure) => figure instanceof Monster && figure.isAllied)
      .map((figure) => (figure as Monster).name);
    scenario.drawExtra = gameManager.game.figures
      .filter((figure) => figure instanceof Monster && figure.drawExtra)
      .map((figure) => (figure as Monster).name);
    scenario.objectives = gameManager.game.figures
      .filter((figure) => figure instanceof ObjectiveContainer)
      .map((figure) => {
    ...
    mailto +=
      '?subject=Submit Scenario #' +
      scenario.index +
      ' ' +
      scenario.name +
      ' (' +
      settingsManager.getLabel('data.edition.' + scenario.edition) +
      ')';
  ...
  issueMail(type: string, text: string): string {
    ...
    mailto +=
      '?subject=[GHS v' + packageJson.version + '] ' + settingsManager.getLabel('tools.feedback.reportIssue.type.' + type + '.subject');
    ...
    mailto +=
      '&body=' +
      settingsManager.getLabel('tools.feedback.reportIssue.type.' + type + '.hint') +
      '%0D%0A%0D%0A' +
      text.replaceAll('\n', '%0D%0A') +
      '%0D%0A%0D%0A' +
      enviroment;
