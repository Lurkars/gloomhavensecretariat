...
export class ImbuementManager {
  ...
  update() {
    this.available =
      gameManager.gh2eRules() &&
      gameManager.game.party.conclusions.some((s) => s.edition == 'gh2e' && s.index == '48.2') &&
      (gameManager.game.party.conclusions.some((s) => s.edition == 'gh2e' && s.index == '16.5') ? 'advanced' : true);
    ...
    if (!settingsManager.settings.gh2eImbuementKeep || !this.imbuement) {
      ...
      if (
        this.imbuement &&
        gameManager.game.monsterAttackModifierDeck.cards.find((am) => am.type == AttackModifierType.advancedImbue) != undefined
      ) {
  ...
  disable(attackModifierDeck: AttackModifierDeck) {
    attackModifierDeck.cards = attackModifierDeck.cards.filter(
      (am) => am.type != AttackModifierType.imbue && am.type != AttackModifierType.advancedImbue
    );
