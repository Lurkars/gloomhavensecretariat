...
import {
  AttackModifier,
  AttackModifierType,
} from "../model/data/AttackModifier";
import {
  Condition,
  ConditionName,
  ConditionType,
} from "../model/data/Condition";
import { Element, ElementState } from "../model/data/Element";
import {
  AdditionalIdentifier,
  CountIdentifier,
  Identifier,
} from "../model/data/Identifier";
import {
  ItemData,
  ItemEffect,
  ItemEffectType,
  ItemFlags,
  ItemSlot,
} from "../model/data/ItemData";
import {
  LootClass,
  LootType,
  getLootClass,
  herbResourceLootTypes,
} from "../model/data/Loot";
...
export class ItemManager {
  ...
  itemEditions(edition: string | undefined = undefined): string[] {
    return gameManager.editionData
      .filter(
        (editionData) =>
          settingsManager.settings.editions.indexOf(editionData.edition) !=
            -1 &&
          (!edition ||
            edition == editionData.edition ||
            gameManager
              .editionExtensions(edition)
              .indexOf(editionData.edition) != -1 ||
  ...
  getItems(edition: string | undefined, all: boolean = false): ItemData[] {
    return gameManager
      ...
      .filter(
        (itemData) =>
          (all && (!edition || itemData.edition == edition)) ||
          this.isItemAvailable(itemData, edition),
      );
  }
  ...
  isItemAvailable(
    ...
    withUnlocks: boolean = true,
  ): boolean {
    if (!this.game.party.campaignMode) {
      return (
        !edition ||
        itemData.edition == edition ||
        gameManager.editionExtensions(edition).indexOf(itemData.edition) != -1
      );
    }
    ...
    if (
      ...
      if (
        withUnlocks &&
        this.game.party.unlockedItems.find(
          (identifier) =>
            identifier.name == "" + itemData.id &&
            identifier.edition == itemData.edition,
        )
      ) {
        return true;
      }
      ...
      if (
        itemData.unlockProsperity > 0 &&
        itemData.unlockProsperity <= gameManager.prosperityLevel()
      ) {
        return true;
      }
      ...
      if (
        ...
        this.game.party.buildings.find(
          (buildingModel) =>
            buildingModel.name == itemData.requiredBuilding &&
            buildingModel.level >= itemData.requiredBuildingLevel,
        )
      ) {
        return true;
      }
    ...
    if (settingsManager.settings.fhGhItems && gameManager.fhRules()) {
      if (itemData.edition == "gh") {
        const tradingPost =
          ...
          this.game.party.buildings.find(
            (buildingModel) =>
              buildingModel.name == "trading-post" &&
              buildingModel.state != "wrecked",
          );
        if (tradingPost) {
          if (
            typeof itemData.id === "number" &&
            tradingPost.level >= 2 &&
            [21, 37, 53, 93, 94, 106, 115].indexOf(itemData.id) != -1
          ) {
            return true;
          } else if (
            typeof itemData.id === "number" &&
            tradingPost.level >= 3 &&
            [
              46, 83, 84, 85, 86, 87, 88, 102, 110, 111, 120, 121, 122, 123,
              126, 128,
            ].indexOf(itemData.id) != -1
          ) {
            return true;
          } else if (
            typeof itemData.id === "number" &&
            tradingPost.level >= 4 &&
            [
              17, 35, 47, 51, 62, 74, 77, 78, 79, 80, 81, 82, 117, 118, 119,
              127, 129, 131,
            ].indexOf(itemData.id) != -1
          ) {
            return true;
        ...
        return (
          typeof itemData.id === "number" &&
          [10, 25, 72, 105, 109, 116].indexOf(itemData.id) != -1
        );
        ...
        if (
          this.game.party.scenarios.find(
            (model) =>
              model.edition == "fh" && model.index == "82" && !model.group,
          )
        ) {
        ...
        if (
          ...
          this.game.party.buildings.find(
            (buildingModel) =>
              buildingModel.name == "enhancer" &&
              buildingModel.level == 4 &&
              buildingModel.state != "wrecked",
          )
        ) {
    ...
    return gameManager
      ...
      .find(
        (itemData) =>
          ...
          itemData.id == id &&
          itemData.edition == edition &&
          (all || this.isItemAvailable(itemData, edition)),
      );
  }
    ...
    return this.game.figures
      ...
      .filter(
        (identifier) =>
          ...
          identifier.edition == item.edition,
      ).length;
  }
  ...
  canBuy(item: ItemData, character: Character, cost: number = 0): boolean {
    if (
      character.tags.indexOf("new-character") == -1 &&
      this.buyingDisabled()
    ) {
      return false;
    }
    ...
    return (
      (item.cost &&
        item.cost + this.pricerModifier() + cost <= character.progress.gold &&
        this.canAdd(item, character)) ||
      false
    );
  }
    ...
    if (item.resources) {
      Object.keys(item.resources).forEach((key) => {
        ...
        const requiredResource =
          (item.resources[lootType] || 0) + (resources[lootType] || 0);
        ...
        if (getLootClass(lootType) == LootClass.herb_resources) {
          canCraft =
            canCraft &&
            ((character.progress.loot[lootType] || 0) >= requiredResource ||
        ...
        if (itemId != item.id) {
          ...
          if (!requiredItem) {
            ...
            if (!this.owned(requiredItem, character)) {
              canCraft =
                canCraft &&
                (this.canCraft(requiredItem, character, item.resources || {}) ||
                  this.canBuy(requiredItem, character, item.cost));
            }
          }
        }
      });
  ...
  canDistill(item: ItemData) {
    if (gameManager.buildingsManager.distillAvailable) {
      if (item.resourcesAny && item.resourcesAny.length) {
        ...
        const resources: LootType[] = Object.keys(item.resources).map(
          (value) => value as LootType,
        );
        return resources.every(
          (resource) => herbResourceLootTypes.indexOf(resource) != -1,
        );
      }
    }
    return false;
      ...
      Object.keys(item.resources).forEach((key) => {
        ...
        if (getLootClass(lootType) == LootClass.herb_resources) {
          if ((character.progress.loot[lootType] || 0) >= requiredResource) {
            ...
            this.game.party.loot[lootType] =
              (this.game.party.loot[lootType] || 0) - requiredResource;
          }
        } else {
          character.progress.loot[lootType] =
            (character.progress.loot[lootType] || 0) - requiredResource;
        ...
        if (itemId != item.id) {
          ...
          if (!requiredItem) {
            ...
            const charItem = character.progress.items.find(
              (identifier) =>
                identifier.name == "" + requiredItem.id &&
                identifier.edition == requiredItem.edition,
            );
            if (charItem) {
              const index = character.progress.items.indexOf(charItem);
              character.progress.items.splice(index, 1);
              character.progress.equippedItems =
                character.progress.equippedItems.filter(
                  (identifier) =>
                    identifier.name != "" + requiredItem.id ||
                    identifier.edition != requiredItem.edition,
                );
      ...
      if (countAvailable + assigend < itemData.count) {
        ...
        if (!unlocked) {
          gameManager.game.party.unlockedItems.push(
            new CountIdentifier("" + itemData.id, itemData.edition, 1),
          );
        } else {
          unlocked.count += 1;
  ...
  countAvailable(item: ItemData): number {
    ...
    if (!this.isItemAvailable(item, item.edition, false)) {
      const unlocked = gameManager.game.party.unlockedItems.find(
        (identifier) =>
          +identifier.name == item.id && identifier.edition == item.edition,
      );
  ...
  isEquipped(item: ItemData, character: Character): boolean {
    return (
      character.progress.equippedItems.find(
        (identifier) =>
          identifier.name == "" + item.id && identifier.edition == item.edition,
      ) != undefined
    );
  ...
  toggleEquippedItem(item: ItemData, character: Character, force: boolean) {
    let equippedItems: ItemData[] = character.progress.equippedItems
      .map((identifier) =>
        this.getItem(identifier.name, identifier.edition, true),
      )
      .filter((itemData) => itemData)
      .map((itemData) => itemData as ItemData);
    const equipIndex = equippedItems.indexOf(item);
    const fhItem3Equipped = equippedItems.find(
      (itemData) => itemData.id == 3 && itemData.edition == "fh",
    );
    ...
    if (force && equipIndex == -1) {
      ...
      const smallEquipped = equippedItems.filter(
        (itemData) => itemData.slot == ItemSlot.small,
      ).length;
      ...
      const drifterAdditionalHand =
        character.name == "drifter" &&
        character.edition == "fh" &&
        character.progress.perks[11];
      if (item.slot == ItemSlot.small) {
        let allowed = Math.ceil(character.level / 2);
        if (
          equippedItems.find(
            (itemData) =>
              (itemData.id == 16 && itemData.edition == "gh") ||
              (itemData.id == 60 && itemData.edition == "fh"),
          )
        ) {
          allowed += 2;
        }
        if (
          equippedItems.find(
            (itemData) => itemData.id == 132 && itemData.edition == "fh",
          )
        ) {
          allowed += 1;
        }
        ...
        if (
          gameManager.trialsManager.favorsEnabled &&
        ...
        if (
          equippedItems.filter((itemData) => itemData.slot == item.slot)
            .length >= allowed
        ) {
          const equipped = equippedItems.find(
            (itemData) => itemData.slot == item.slot,
          );
          if (equipped) {
            equippedItems.splice(equippedItems.indexOf(equipped), 1);
          }
        ...
        const twoHand = equippedItems.find(
          (equipped) => equipped.slot == ItemSlot.twohand,
        );
        ...
        if (
          equippedItems.filter((equipped) => equipped.slot == item.slot)
            .length > (drifterAdditionalHand ? (twoHand ? 0 : 2) : 1)
        ) {
          const equipped = equippedItems.find(
        ...
        if (drifterAdditionalHand) {
          while (
            equippedItems.filter(
              (equipped) => equipped.slot == ItemSlot.onehand,
            ).length > 1
          ) {
            const equipped = equippedItems.find(
              (equipped) => equipped.slot == ItemSlot.onehand,
    ...
    character.progress.equippedItems = equippedItems.map(
      (itemData) =>
        new AdditionalIdentifier(itemData.id + "", itemData.edition),
    );

    if (
      item.minusOne &&
      !gameManager.characterManager.ignoreNegativeItemEffects(character)
    ) {
      character.attackModifierDeck =
        gameManager.attackModifierManager.buildCharacterAttackModifierDeck(
          character,
        );
    ...
    const fhItem3StillEquipped = equippedItems.find(
      (itemData) => itemData.id == 3 && itemData.edition == "fh",
    );

    if (fhItem3Equipped && !fhItem3StillEquipped) {
  ...
  drawRandomItem(
    ...
    let availableItems = this.getItems(undefined, true).filter(
      (itemData) =>
        ...
        (from == -1 ||
          (typeof itemData.id === "number" && itemData.id >= from)) &&
        ...
        (itemData.edition == edition ||
          gameManager.editionExtensions(edition).indexOf(itemData.edition) !=
            -1),
    );
  ...
  applyEquippedItemEffects(character: Character) {
    ...
    character.progress.equippedItems.forEach((identifier) => {
      const item = gameManager.itemManager.getItem(
        identifier.name,
        identifier.edition,
        true,
      );
    ...
    switch (effect.type) {
      case ItemEffectType.heal:
        ...
        gameManager.entityManager.addCondition(
          character,
          character,
          new Condition(ConditionName.heal, heal),
        );
        gameManager.entityManager.applyCondition(
          character,
          character,
          ConditionName.heal,
          true,
      ...
      case ItemEffectType.condition:
        const condition: ConditionName = ("" + effect.value).split(
          ":",
        )[0] as ConditionName;
        let value: number =
          ("" + effect.value).split(":").length > 1
            ? +("" + effect.value).split(":")[1]
            : 1;
        ...
        if (condition == ConditionName.bless) {
          ...
          for (let i = 0; i < value; i++) {
            if (
              gameManager.attackModifierManager.countUpcomingCurses(false) < 10
            ) {
              gameManager.attackModifierManager.addModifier(
                character.attackModifierDeck,
                new AttackModifier(AttackModifierType.curse),
      ...
      case ItemEffectType.refreshSpent:
        character.progress.equippedItems.forEach((identifier) => {
          if (
            identifier.tags &&
            identifier.tags.indexOf(ItemFlags.spent) != -1
          ) {
            identifier.tags = identifier.tags.filter(
              (tag) => tag != ItemFlags.spent,
            );
          }
        });
        break;
      case ItemEffectType.removeNegativeConditions:
        ...
        character.entityConditions = character.entityConditions.filter(
          (entityCondition) =>
            negativeConditions.indexOf(entityCondition.name) == -1,
        );
        break;
    }
  }
