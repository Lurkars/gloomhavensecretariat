...
export const sortAction = function (action) {
  ...
  if (action.type === 'area' && action.value) {
    ...
    action.value.split('|').forEach((hexString) => {
      let groups = new RegExp(/^\((\d+),(\d+),(active|target|conditional|ally|blank|enhance|invisible|custom)(\:([\w-]*))?\)$/).exec(
        hexString
      );
      if (groups == null) {
        return null;
    ...
    action.value = hexes
      .map(
        (actionHex) => '(' + actionHex.x + ',' + actionHex.y + ',' + actionHex.type + (actionHex.value ? ':' + actionHex.value : '') + ')'
      )
      .join('|');
  }
